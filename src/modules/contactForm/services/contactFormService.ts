...
export default class ContactFormService {
    ...
    async getAllContacts() : Promise<Contact[]> {
        return await this.getAllAndFlatten<Contact>(ContactFormService.CONTACTS_COLLECTION);
    }

    updateContactStatus(contactId : string, status : number) : Promise<Contact> {
        return this.db.runTransaction(async transaction => {
            const contactDoc = await transaction.get(this.db.collection(ContactFormService.CONTACTS_COLLECTION).doc(contactId))
            const updatedOption = {
                ...contactDoc.data(),
                id: contactDoc.id,
                status: status
